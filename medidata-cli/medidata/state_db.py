...
def get_conn() -> sqlite3.Connection:
    ...
    # Anyone with a state.db created before refresh-token support was added
    # won't have this column yet — add it in place rather than forcing a
    # re-login just to pick up the schema change.
    try:
        conn.execute("ALTER TABLE auth_session ADD COLUMN refresh_token TEXT")
...
def save_session(access_token: str, profile: dict, base_url: str, refresh_token: str | None = None) -> None:
    with state_conn() as conn:
        conn.execute("DELETE FROM auth_session")
        conn.execute(
            """INSERT INTO auth_session
               (id, access_token, refresh_token, profile_id, full_name, email, role,
